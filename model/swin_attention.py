...
from model.conv_attention_2 import attn_block
from model.twins_attention_orig import TwinSVT
class tattn_block(nn.Module):
    def __init__(self, in_channels, out_channels, t_num_heads=4, num_point=25, block_size=41, t_depth=1, t_patch=2, global_k=5, is_last=False, window_size=120, t_sr_ratio=8):
        super(tattn_block, self).__init__()

        # self.layers = nn.Sequential(
        #         PatchEmbedding(dim = in_channels, dim_out = out_channels, patch_size = t_patch),
